...
21. Zebra POS v2 handheld cashier (BUILT as `/zebra` in THIS app — first working version of the separate "Zebra POS v2" track, see the Zebra page in Notion): a companion cashier UI purpose-built for the Zebra TC53 handheld (6" 1080x2160 portrait, Android 13, Chrome). Same database, same APIs, no new backend. Scanning is DataWedge KEYBOARD-WEDGE, not the camera: the device types the barcode into whatever is focused and sends Enter, so the page is built around one always-focused input (`#wedge-input`) that snaps focus back after every tap/modal — never interfere with or replace DataWedge, build around it. On Enter, an all-digit value runs the proven barcode flow (found fixed-price → straight onto the cart, found measured → quantity pad, not found → Quick Add auto-opens with the barcode attached, decision 13); text with letters runs the name search. Checkout adds a PAYMENT step that doubles as the decision-11 confirmation guard: it shows the total plus the shop's REAL static Fonepay QR (`static/fonepay-static-qr.jpg` — a photo of the physical standee QR; NEVER generate a QR programmatically, only Fonepay's system can produce valid QR data; a graceful "not installed yet" message shows until the photo is added), and the sale is saved only when staff tap PAYMENT RECEIVED after visually confirming payment (or taking cash). `renderPaymentQr(saleTotal)` in `static/zebra.js` is the SINGLE marked swap point (TODO(fonepay-dynamic-qr)) for the future Fonepay Dynamic QR API + webhook confirmation — blocked on Fonepay/bank credentials, tracked in Notion. Also on the page: the same group/pinned quick-tap buttons, price override, bilingual toggle, BS-date header, and SAVE CART — named parked carts (customer name) held in localStorage per device, resumable by chip; parked carts are unfinished business and are never sent to the server (server-persisted carts + Chromebook live-sync remain a future phase of the Zebra track). Files: `templates/zebra.html`, `static/zebra.js`, `static/zebra.css` (loaded after `style.css`, reusing its modal/numpad/bill/tile components).
...
- `templates/zebra.html`, `static/zebra.js`, `static/zebra.css` — the `/zebra` handheld cashier for the Zebra TC53 (decision 21); expects the static Fonepay QR photo at `static/fonepay-static-qr.jpg` (not in git yet)
...
Zebra handheld cashier at `/zebra` (decision 21 — for the Zebra TC53, same database/APIs):
1. Always-focused barcode input fed by DataWedge keyboard-wedge scanning (typed code + Enter); doubles as name search
2. Same lookup-and-decide flow as the camera cashier: add to cart / quantity pad / auto Quick Add
3. Named saved carts (park + resume, localStorage per device)
4. Payment step: sale total + static Fonepay QR image + manual "PAYMENT RECEIVED" confirmation; `renderPaymentQr()` is the marked swap point for the future Fonepay Dynamic QR API
