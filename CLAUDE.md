...
products (store.db):
id INTEGER PK, barcode TEXT nullable, name TEXT (canonical English, used on sales), category TEXT (grocery/weighed/lpg/stationery/cosmetics/other), price REAL, is_weighed BOOLEAN, unit TEXT (kg/piece/packet/bottle), active BOOLEAN (soft delete), weighed_group TEXT nullable (the product's cashier GROUP name — any group from the `groups` table, weighed or fixed; kept its historical column name — see decision 19), name_ne TEXT nullable (optional Nepali display name, cashier-only — see decision 15), pinned INTEGER default 0 (fixed-price product shown as its own one-tap cashier button — see decision 17), image_path TEXT nullable (filename of the optional product photo stored under data/images/ — see decision 18)
...
3. User-defined quick-tap GROUP buttons (see decision 19): weighed groups (green, e.g. Rice/Dal) → variety picker → weight pad; fixed-price groups (orange, e.g. LPG, Oil) → tap-to-add list. Managed self-serve on the admin Groups page.
